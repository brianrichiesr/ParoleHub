...
const DisplayCountyResult = (props) => {
    // Set the state of 2 consts with useState.
    const [resource, setResource] = useState({});
    const [error, setError] = useState('');
    // Declare variables whose values depend on the url.
    const path = window.location.href.split('/');
    const county = path[5].split('%20').join(' ');
    const id = path[6];
    const resultNum = path[7];
    // Sets a limit based on the number of search results
    let limit = 0;
    let currentCountyList = JSON.parse(window.localStorage.getItem('currentCountyList'));
    for (let idx in currentCountyList) {
        limit++;
    }
    /* If there are no results, sends the user back to the 'CountyResults' page to select another option. */
    if (limit === 0) {
    ...
    const previousResult = (e) => {
        ...
        let newPath = [...path];
        newPath[6] = currentCountyList[newResult].id;
        newPath[7] = currentCountyList[newResult].result;
                ...
                <div id='mb1'>
                    <h1 id='searchH1'>
                        <span id='bannerSpan'>
                            ...
                            search for {'"' + (county.length > 15 ? county.toLowerCase().slice(0, 15) + '...' : county.toLowerCase()) + '"'}
                        </span>
                    </h1>
                </div>
                ...
                {props.userStatus === 'user' ?
                    <div id='mb4'>
                        <span id='displayLiButtonDiv'>
                            {/* Link that will return user back to the list of results from the county search.*/}
                            <Link
                                id={'returnToListButton'}
                                onClick={() => boolTrue()}
                                to={`/${'search/county/' + county + '#' + id}`}
                                key={2}
                    ...
                    : <div id='mb3'>
                        ...
                        <span id='displayLiButtonDiv'>
                            {/* Link that will take user to the 'CountyResourceUpdate' page.*/}
                            <Link
                                id={'updateResourceButton'}
                                to={`/${'resources/update/' + id + '/' + county + '/' + resultNum}`}
                                key={3}
...
export default DisplayCountyResult;