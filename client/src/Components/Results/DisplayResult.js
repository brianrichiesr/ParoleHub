...
const DisplayResult = (props) => {
    ...
    const zipcode = path[5];
    const id = path[6];
    const distance = path[7];
    const resultNum = path[8];
     // Sets a limit based on the number of search results
    let limit = 0;
    let currentList = JSON.parse(window.localStorage.getItem('currentList'));
    for (let idx in currentList) {
        limit++;
    }
     /* If there are no results, sends the user back to the 'ResourceResults' page to select another option. */
    if (limit === 0) {
    ...
    const previousResult = (e) => {
        ...
        let newPath = [...path];
        newPath[6] = currentList[newResult].id;
        newPath[7] = currentList[newResult].distance;
        newPath[8] = currentList[newResult].result;
                ...
                <div id='mb1'>
                    ...
                    <div id='mb4'>
                        <span id='displayLiButtonDiv'>
                            {/* Link that will return user back to the list of results from the zip code search.*/}
                            <Link
                                id={'returnToListButton'}
                                onClick={() => boolTrue()}
                                to={`/${'search/zipcodes/' + zipcode + '#' + id}`}
                                key={2}
                    ...
                    :<div id='mb3'> 
                        ...
                        <span id='displayLiButtonDiv'>
                            {/* Link that will take user to the 'ResourceUpdate' page.*/}
                            <Link
                                id={'updateResourceButton'}
                                to={`/${'resources/update/' + id + '/' + zipcode + '/' + distance + '/' + resultNum}`}
                                key={3}
            ...
            <div id='displayDiv'>
                ...
                <div id='displayLiDiv'>
                    ...
                    >
                        ...
                        <p>
                                <span className='importantText'>Distance From Chosen Zip Code:</span> Approx.
                                <span className='resultSpan'>
                                    {' ' + distance}
                                </span> miles
                            </p>
