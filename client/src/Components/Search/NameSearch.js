...
/* Functional component that creates an input field for 'resourceNames' and a button to initiate a search based on the 'resourceName' the user inputs. Will only receive props if called from the 'NameResults' page. */
const NameSearch = (props) => {
    ...
    const banner = 'name searched';
    /* Function that takes one argument (the event), and sends the user to a url based on the 'resourceName' the user inputs into the input field. */
    const searchByResourceName = (e) => {
        // Prevents the form from submitting normally.
        e.preventDefault();
        // Declares a const equal to the value of the input.
        let resourceName = document.forms.searchedByName.resourceName.value;
        /* If props.banner does not exist, send the user to the current url plus / and the 'resourceName' that the user inputs. */
        if (!props.banner) {
            props.history.push('name/' + resourceName)
            //  Else replace the last param of the current url with 'resourceName' and send user to that address. 
        } else {
            const list = document.getElementById('filters');
            list.className = 'asideLeft';
            props.history.push(resourceName);
        };
        document.forms.searchedByName.resourceName.value = '';
    };
    /* If the list of filters is showing and the user clicks anything in the list, do nothing. Else, hide the list of filters. */
    const closeFilter = (e) => {
        if (e.type === 'click' && (e.target.className.includes('noClick'))) return;
        setFilterOpen(false);
    }
    ...
    useEffect(() => {
        ...
        // App will remove the event listeners when the component unmounts.
        return () => {
    ...
    /* Functions that run when the state of 'filterOpen' changes. */
    useEffect(() => {
        const list = document.getElementById('filters');
            ...
            <div id='mainBody'>
                ...
                <div id='mb2'>
                    <form
                        id='searchedBy'
                        name='searchedByName'
                        onSubmit={(event) => searchByResourceName(event)}
                    >
                        <input id='resourceName' name='resourceName' placeholder='input name here' required />
                        <button id="searchButton" className="post" type="submit">search name</button>
                    </form>
