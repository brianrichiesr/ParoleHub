...
/* Functional component that creates an input field for Counties and a button to initiate a search based on the County the user inputs. Will only receive props if called from the 'CountyResults' page. */
const CountySearch = (props) => {
    ...
    const banner = 'county searched';
    /* Function that takes one argument (the event), and sends the user to a url based on the County the user inputs into the input field. */
    const searchByCounty = (e) => {
        // Prevents the form from submitting normally.
        e.preventDefault();
        // Declares a const equal to the value of the input.
        let county = document.forms.searchedByCounty.county.value;
        /* If props.banner does not exist, send the user to the current url plus / and the county that the user inputs. */
        if (!props.banner) {
            props.history.push('county/' + county)
            //  Else replace the last param of the current url with 'county' and send user to that address. 
        } else {
            const list = document.getElementById('filters');
            list.className = 'asideLeft';
            props.history.push(county);
        };
        document.forms.searchedByCounty.county.value = '';
    };
        ...
        <div>
            <div id='mainBody'>
                ...
                <div id='mb2'>
                    <form
                        id='searchedBy'
                        name='searchedByCounty'
                        onSubmit={(event) => searchByCounty(event)}
                    >
                        <input id='county' name='county' placeholder='input county here' required />
                        <button id="searchButton" className="post" type="submit">search county</button>
                    </form>
...
export default withRouter(CountySearch);
