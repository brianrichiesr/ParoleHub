...
const NameResourceUpdate = () => {
    // Declares some consts with some relevant url information.
    const path = window.location.href.split('/');
    const id = path[5];
    const resourceName = path[7];
    const resultNum = path[6];
        ...
        <div className='resourceForm'>
            ...
            >
                ...
                >
                    ...
                    >
                        {resourceDelete ? <Link
                            to={`/${'search/name/' + resourceName}`}
                            key={98}
                            className={'reactButton navLink'}
                        >
                            Return To List
                        </Link> : <Link
                            to={`/${'search/name/' + resourceName + '/' + id + '/' + resultNum}`}
                            key={99}
                            className={'reactButton navLink'}
                        >
                            back to resource
...
export default NameResourceUpdate;
